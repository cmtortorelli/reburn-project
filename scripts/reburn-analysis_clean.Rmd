...
# count_unique_reburns = data_lowmod %>%
...
thin_sw = left_join(thin_sw, data_lowmod_sw %>% mutate_at(vars(lat, long), list(~ round(., 5))))

thin_nm = read.csv(datadir("prelim-outputs/points/thinned_data_nm.csv"))%>%
  mutate_at(vars(lat, long), list(~ round(., 5)))
thin_nm = left_join(thin_nm, data_lowmod_nm  %>%
  mutate_at(vars(lat, long), list(~ round(., 5))))
...
thin_ca = read.csv(datadir("prelim-outputs/points/thinned_data_ca.csv"))%>%
  mutate_at(vars(lat, long), list(~ round(., 5)))
thin_ca = left_join(thin_ca, data_lowmod_ca  %>%
  mutate_at(vars(lat, long), list(~ round(., 5))))
...
thin_wm = bind_rows(thin_wm1, thin_wm2) %>%
  mutate_at(vars(lat, long), list(~ round(., 5)))
thin_wm = left_join(thin_wm, data_lowmod_wm  %>%
  mutate_at(vars(lat, long), list(~ round(., 5))))
...
preds_dfc_func = function(df, fit){
  ...
  df_hotdry = data.frame(count_burn = 0, 
                       ...
                       PRISM_ppt_ = quantile(df$PRISM_ppt_, 0.1),
                       PRISM_tmea = quantile(df$PRISM_tmea, 0.90),
                        #control for other vars - mean
                       cbi_initial = 1.5, heat_load = 0, TPI_1024 = 0,
                       vpd_perc = .9, erc_perc = .9,vs_perc = .5,
                       # afg_rebrn = 0, pfg_rebrn = 0,
...
library(ggridges)
data_thin_bps %>% 
  filter(CT_lumped_codes2 %in% common_bps) %>%
  ggplot(aes(x = time_betwe, y = CT_lumped_codes2, group = CT_lumped_codes2, fill = CT_lumped_codes2))+
  geom_density_ridges()+
  facet_grid(cols= vars(ecoregion), scales = "free")+
  theme_bw(13)+scale_fill_manual(values = c("#771155", "#AA4488", "#CC99BB", "#114477", "#4477AA", "#77AADD", "#117777", "#44AAAA", "#77CCCC", "#117744", "light grey", "#88CCAA", "#777711", "#AAAA44", "#DDDD77", "#774411", "#AA7744", "#DDAA77", "#771122", "#AA4455", "#DD7788"))
...
count_bps = data_thin_bps %>% group_by(CT_lumped_codes2) %>% summarise(n = n())
...
data_thin_bps %>%
  ggplot(aes(x = n, y = ecoregion, fill = CT_lumped_codes2))+
  geom_bar(position = "fill", stat = "identity")+
  theme_bw(13)+ scale_fill_manual(values = c("#771155", "#AA4488", "#CC99BB", "#114477", "#4477AA", "#77AADD", "#117777", "#44AAAA", "#77CCCC", "#117744", "light grey", "#88CCAA", "#777711", "#AAAA44", "#DDDD77", "#774411", "#AA7744", "#DDAA77", "#771122", "#AA4455", "#DD7788"))
