...
#initial year tree cover - NOT COMPLETE! *I only extracted tree cover for 2002-2021 from GEE*
treeli = tree %>% 
...
cbi = points_gee %>% 
  data.frame() %>%
  dplyr::select(pointid, secondrece, cbi_2021:cbi_1995) %>%
...
#read in facts
facts <- st_read(datadir("raw-data/facts/compiled/facts_compiled_for_points_extract.gpkg"))
...
st_write(facts_int, datadir("prelim-outputs/points/points_buff_facts_int-12-13-22.gpkg"))
...
st_write(points_cleaned_facts, datadir("prelim-outputs/points/points_cleaned_facts-12-13-22.gpkg"))
```
...
mtbs_ids = read.csv(datadir("raw-data/fire_id_tables/mtbs_id_table.csv")) %>%
  ...
  dplyr::select(Fire_ID, Fire_Type) 
...
reburn_fire_id = left_join(reburn_fire_id, fire_id_table) %>%
  ...
  dplyr::select(pointid, reburn_fire_type, reburn_fireID)

initial_fire_id = left_join(initial_fire_id, fire_id_table) %>%
    mutate(initial_fire_type = Fire_Type) %>%
  dplyr::select(pointid, initial_fire_type, initial_fireID)
...
st_write(points_cleaned_facts_perims, datadir("prelim-outputs/points/points_cleaned_facts_perims-12-13-22.gpkg"))
...
st_write(points_fs_include, datadir("prelim-outputs/points/points_for_analysis_12-13-22.gpkg"))
