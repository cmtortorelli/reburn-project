...
add missing data
```{r}
#add missing data - annual and perennial vegetation pre-fire
...
points_reburnyr = data_sub %>% dplyr::select(pointid, mostrecent, secondrece)

#keep only afg data for year prior to reburn
afgl = afg %>% 
...
data_sub1 = full_join(data_sub, data.frame(afgl))
data_sub1 = full_join(data_sub1, data.frame(pfgl))


#add weather percentiles for erc, vs, and vpd 
perc_w = read_sf(datadir("prelim-outputs/points/dob_percentiles.gpkg"))

data_sub1b = st_join(data_sub1, perc_w[,c("erc_perc","vpd_perc","vs_perc")])
```

clean data
```{r}
#clean data
data_sub2 = data_sub1b %>% 
...
data_lowmod %>%
  dplyr::select(-c(reburn_fire_type, initial_fire_type, initial_fireID, reburn_fireID, postmanage10, premanage10, geometry, sample_zones, lat, long)) %>%
...
scale_df = function(df){
  data_scale = scale(df[,c(1,3,5:21,32:33)], scale = TRUE, center = TRUE)
  data_scale_all = cbind(data_scale, df[,c(2,4,22:31,34:40)])
...
gm = function(df){
  ...
  gamfit = gam(cbi_reburn ~ count_burn + #not logit transformed response
                  s(AET, k = 4, bs = "cs") +
                  # s(PRISM_ppt_, k = 4, bs = "cs") +
                  # s(PRISM_tmea, k = 4, bs = "cs") +
                  # s(PRISM_vpdm, k = 5, bs = "cs") +
                  s(cbi_initial, k = 4, bs = "cs") +
                  s(time_betwe, k = 4, bs = "cs") +
                  s(erc_perc, k = 4, bs = "cs") +
                  s(vs_perc, k = 4, bs = "cs") +
                  s(vpd_perc, k = 4, bs = "cs") +
                  s(afg_rebrn, k = 4, bs = "cs") +
                  s(pfg_rebrn, k = 4, bs = "cs") +
                  # s(erc_02_21, k = 4, bs = "cs") +
                  # s(th_02_21, k = 5, bs = "cs") +
                  # s(vpd_02_21, k = 4, bs = "cs") +
                  # s(vs_02_21, k = 4, bs = "cs") +
                  s(heat_load, k = 4, bs = "cs") +
                  s(TPI_1024, k = 4, bs = "cs") +
                  # s(ndvi, k = 5, bs = "cs") +
                  # s(tree_reburn, k = 5, bs = "cs") +
                  ti(time_betwe, by = AET, k = 4, bs = "cs") +
                  # ti(time_betwe, by = PRISM_ppt_, k = 4, bs = "cs") +
                  # ti(time_betwe, by = PRISM_tmea, k = 4, bs = "cs") +
                  ti(time_betwe, by = vpd_02_21, k = 4, bs = "cs") +
                  ti(time_betwe, by = vs_02_21, k = 4, bs = "cs") +
                  ti(time_betwe, by = erc_02_21, k = 4, bs = "cs") +
...
fit3_ca = gm_prism(data_lowmod_ca)
fit3_nm = gm_prism(data_lowmod_nm)
fit3_sw = gm_prism(data_lowmod_sw)
fit3_wm = gm_prism(data_lowmod_wm)
...
data_w_ca <- preds_dfw_func(df = data_lowmod_ca, fit = fit3_ca)
data_w_nm <- preds_dfw_func(df = data_lowmod_nm, fit = fit3_nm)
data_w_sw <- preds_dfw_func(df = data_lowmod_sw, fit = fit3_sw)
data_w_wm <- preds_dfw_func(df = data_lowmod_wm, fit = fit3_wm)
```
climate predictions
```{r}
...
preds_dfc_func = function(df, fit){
  ...
  df_coolwet = data.frame(count_burn = 0, 
                       ...
                       cbi_initial = 0, heat_load = 0, TPI_1024 = 0,
                       vpd_02_21 = 0, erc_02_21 = 0,vs_02_21 = 0,
                       #set time between fires to full range 1-25 years
                       time_betwe = seq(0.1, 2.5, by = .1),
                       climate = "cool/wet",
                       #control for lat long
...
data_c_ca <- preds_dfc_func(df = data_lowmod_ca, fit = fit3_ca)
data_c_nm <- preds_dfc_func(df = data_lowmod_nm, fit = fit3_nm)
data_c_sw <- preds_dfc_func(df = data_lowmod_sw, fit = fit3_sw)
data_c_wm <- preds_dfc_func(df = data_lowmod_wm, fit = fit3_wm)

#plot precip and temp separately
vis.gam(fit3_ca, c)
```
...
pca = ggplot(data = data_w_ca, aes(x = time_betwe, y = cbi_reburn, color = weather)) +
         geom_line() +
           geom_ribbon(aes(ymin = lwr, ymax = upr, fill = weather), alpha = 0.1, colour = NA)+
         theme_bw() +
  ggtitle("California coast")

psw = ggplot(data = data_w_sw, aes(x = time_betwe, y = cbi_reburn, color = weather)) +
         geom_line() +
         geom_ribbon(aes(ymin = lwr, ymax = upr, fill = weather), alpha = 0.1, colour = NA)+
         theme_bw()+
  ggtitle("Southwest")

pnm = ggplot(data = data_w_nm, aes(x = time_betwe, y = cbi_reburn, color = weather)) +
         geom_line() +
           geom_ribbon(aes(ymin = lwr, ymax = upr, fill = weather), alpha = 0.1, colour = NA)+
         theme_bw()+
  ggtitle("Northern mountains")

pwm = ggplot(data = data_w_wm, aes(x = time_betwe, y = cbi_reburn, color = weather)) +
         geom_line() +
           geom_ribbon(aes(ymin = lwr, ymax = upr, fill = weather), alpha = 0.1, colour = NA)+
         theme_bw()+
...
```{r}
pca = ggplot(data = data_c_ca, aes(x = time_betwe, y = cbi_reburn, color = climate)) +
    scale_color_manual(values = c("blue", "orange", "black", "green"))+
    scale_fill_manual(values = c("blue", "orange", "black", "green"))+
...
psw = ggplot(data = data_c_sw, aes(x = time_betwe, y = cbi_reburn, color = climate)) +
    scale_color_manual(values = c("blue", "orange", "black", "green"))+
    scale_fill_manual(values = c("blue", "orange", "black", "green"))+
...
pnm = ggplot(data = data_c_nm, aes(x = time_betwe, y = cbi_reburn, color = climate)) +
  scale_color_manual(values = c("blue", "orange", "black", "green"))+
    scale_fill_manual(values = c("blue", "orange", "black", "green"))+
...
pwm = ggplot(data = data_c_wm, aes(x = time_betwe, y = cbi_reburn, color = climate)) +
    scale_color_manual(values = c("blue", "orange", "black", "green"))+
  scale_fill_manual(values = c("blue", "orange", "black", "green"))+
         geom_line() +
             geom_ribbon(aes(ymin = lwr, ymax = upr, fill = climate), alpha = 0.1, colour = NA)+
