...
```{r}
data = st_read(datadir("prelim-outputs/points/points_for_analysis_1-12-23.gpkg"))  
...
preds_not2scale = c("cbi_reburn", "time_betwe", "ecoregion", "erc_perc", "vpd_perc",  "vs_perc",
                "count_trt_preburn_mechanical", "count_trt_preburn_pfire", "reburn_fire_type",
                "reburn_fireID", "initial_fire_type", "initial_fireID", "lat",
...
gm_prism = function(df, k){
  ...
  gamfit = gam(cbi_reburn ~ count_burn + #not logit transformed response
                  ...
                  s(afg_rebrn, k = k, bs = "cs") +
                  s(pfg_rebrn, k = k, bs = "cs") +
...
fit3_ca = gm_prism(data_lowmod_ca, k = 6)
fit3_nm = gm_prism(data_lowmod_nm, k = 6)
fit3_sw = gm_prism(data_lowmod_sw, k = 6)
fit3_wm = gm_prism(data_lowmod_wm, k = 6)
