...
#explore spatial autocorrelation; semivariance of data
library(gstat)

# v = variogram(cni85_21~1, data=data)
# plot(v)
# vm <- vgm(psill=0.3, model="Gau", nugget=0.01, range=1000)
# plot(v, model = vm)
# 
# fittedm = fit.variogram(v, model=vm) 
```
...
#break into low, moderate, and high initial fire categories
...
#### Logistic tranform "reburn cbi" response variable because its bounded 0.5-3 *not sure if we need to do this if using GAMs?
```{r}
...
```{r}
#scale predictors for model fit - don't scale time between to maintain interpretation
scale_df = function(df){
  data_scale = scale(df[,c(1,3,5:21)], scale = TRUE, center = TRUE)
  data_scale_all = cbind(data_scale, df[,c(2,4,22:32)])
...
preds_dfw_func = function(df, fit){
  ...
  df_w_mod = data.frame(count_burn = 0, 
                       ...
                       time_betwe = seq(0.1, 2.5, by = .1),
                      weather = "mean")
  df_w_low = data.frame(count_burn = 0, 
                       AET = 0, PRISM_ppt_ = 0, PRISM_tmea = 0, 
                       cbi_initial = 0, heat_load = 0, TPI_1024 = 0,
                       #set weather variables to 1st quantile value  
                       vpd_02_21 = quantile(df$vpd_02_21, 0.25),
                       erc_02_21 = quantile(df$erc_02_21, 0.25),
                       vs_02_21 = quantile(df$vs_02_21, 0.25),
                       #set time between fires to full range 1-25 years
                       time_betwe = seq(0.1, 2.5, by = .1), 
                      weather = "low")
  
  data_w = rbind(df_w_high, df_w_low, df_w_mod)
  
  #predict to new data
  p = predict.gam(fit, newdata = data_w)
...
data_w_ca <- preds_dfw_func(df = data_lowmod_ca, fit = fit_ca)
data_w_nm <- preds_dfw_func(df = data_lowmod_nm, fit = fit_nm)
data_w_sw <- preds_dfw_func(df = data_lowmod_sw, fit = fit_sw)
data_w_wm <- preds_dfw_func(df = data_lowmod_wm, fit = fit_wm)
```
...
vis.gam(fit_ca, view=c('time_betwe', 'AET'), n.grid=50, theta=35, phi=32, zlab="", too.far=0.1)
vis.gam(fit_nm, view=c('time_betwe', 'AET'), n.grid=50, theta=35, phi=32, zlab="", too.far=0.1)
