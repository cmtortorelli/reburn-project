...
thin_wm = read.csv(datadir("prelim-outputs/points/thinned_data_wm.csv")) %>%
  mutate_at(vars(lat, long), list(~ round(., 5)))
...
fit3_ca = gm_prism(data_lowmod_ca, k = 5)
fit3_nm = gm_prism(data_lowmod_nm, k = 5)
fit3_sw = gm_prism(data_lowmod_sw, k = 5)
fit3_wm = gm_prism(data_lowmod_wm, k = 5)
...
range_time = data_lowmod_ca %>% filter(erc_perc >= )
