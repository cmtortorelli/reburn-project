...
ownership = st_transform(ownership, st_crs(mtbs_west))  #set crs equal to bps layer
fs = ownership[ownership$OWNERCLASSIFICATION == "USDA FOREST SERVICE",]

#convert mtbs data to sf object for intersection
mtbs_west_sf = sf::st_as_sf(mtbs_west)

#intersect with mtbs 
mtbs_fs_int = st_intersection(mtbs_west_sf, fs)

#subset mtbs_west so that only fires that intersect FS land are included in severity analysis
...
writeVector(mtbs_west_fs_sub, file = datadir("/raw-data/fire-perims/mtbs_perimeter_data/mtbs_west_fs_sub.shp"))
