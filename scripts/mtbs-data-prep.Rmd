...
writeVector(mtbs_west_fs_sub, file = datadir("/raw-data/fire-perims/mtbs_perimeter_data/mtbs_west_fs_nps_sub.shp"), overwrite = TRUE)

# plot(mtbs_fs_int[1])
# plot(ownership[1], add = T, color = "red")
```

Keep only perimeters that were included in reburns
```{r}
...
z <- rast(mtbs_west_fs_sub_reburn)
...
for (i in 1:length(bz)){
  zone_list[[i]] <- terra::intersect(mtbs_west_fs_sub_reburn, bz[i,])
}
...
#save zones
writeVector(z, file = datadir("/prelim-outputs/analysis_zones9/analysis_zones9.gpkg"), overwrite = TRUE)
```
