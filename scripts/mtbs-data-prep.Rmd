...
mtbs_west_fs_sub3 = mtbs_west_fs_sub2[-(mtbs_west_fs_sub2$Fire_ID == 'CA4009112093120200817'), ]
```

Separate mtbs data from 2021 data
```{r}
#remove 2021 fires from data
perims_sub_21 = mtbs_west_fs_sub3[mtbs_west_fs_sub3$Fire_Year == 2021,]
mtbs_west_fs_sub_no21 = mtbs_west_fs_sub3[mtbs_west_fs_sub3$Fire_Year < 2021,]
```

Remove incomplete fire perimeters from 2021
```{r}
...
#turn of s2 processing to avoid spherical geometry errors
sf::sf_use_s2(FALSE)
#fix geometry
perims_sub_21 = st_make_valid(perims_sub_21)



perims21_intersect <- st_intersection(perims_sub_21)%>% 
  mutate(id_int=seq(from=1, to=nrow(.), by=1)) %>%
  st_collection_extract(perims21_intersect)

perims21_intersect_values = perims21_intersect %>%
...
perims21int2 = st_intersects(perims_ag) #overlaps are 13,14 & 5,9
...
# perims_sub_21 = st_make_valid(perims_sub_21)
# 
# parts <- st_cast(st_union(perims_sub_21, by_feature = TRUE),"POLYGON")
# #
# clust <- data.frame(st_intersects(perims_sub_21, parts))
# 
# #
# diss <- cbind(perims_sub_21, clust) %>%
#   group_by(clust) %>%
...
# perims21_join = st_join(perims_sub_21, join = st_intersects)
...
#   subset(sum != 0) %>%
#       dplyr::select(-sum) %>%
#   pivot_wider(names_from = row.id,
#               values_from = col.id) %>%
#     t() %>%
#   data.frame() %>%
#   mutate(rn = as.integer(as.numeric(rownames(.)))) %>%
#         mutate(merge_id = rowSums(., na.rm = TRUE)) #add unique ID for aggregating intersecting perimeters

# # parts <- st_cast(st_union(perims_sub_21),"POLYGON")
# # 
# clust <- unlist(st_intersects(perims_sub_21, parts))
# # 
# diss <- cbind(perims_sub_21, clust) %>%
#   group_by(clust) %>%
#   summarize(box = paste(box, collapse = ", "))
...
# perims21int = perims21int[colnames(perims21int),]
#   summarise_all(sum, na.rm = TRUE) 
# 
# #add perimeter ID to mtbs_west_sub to index from the reburn dataframe
# perims21_overlap = perims_sub_21 %>%
#   mutate(index = 1:nrow(perims_sub_21)) %>%
#   subset(index %in% perims21int$row.id)
# 
# 
# 
# #union
# perims21_union = st_union(perims_sub_21, by_feature = TRUE)
# perims21_union2 = st_union(perims_sub_21, by_feature = FALSE)
# # r = rast(perims_sub_21, resolution = 30, crs = "epsg:5070")
# 
# perims21_union3 = st_union(st_combine(perims_sub_21))

# perims_sub_21_union = union(perims_sub_21)
# perims_sub_21_union$sum <- rowSums(data.frame(perims_sub_21_union))
# plot(perims_sub_21_union, "sum")
# 
# perims_sub_21_ag = aggregate(perims_sub_21, dissolve = FALSE)

# perims_sub_21_aggregate = aggregate(perims_sub_21, dissolve = TRUE)
# 
# perims_sub_21 = sf::st_as_sf(perims_sub_21)
# 
# #turn of s2 processing to avoid spherical geometry errors
...
# perims_sub_21 = st_make_valid(perims_sub_21)
# 
# #re-project for st_union to planar coordinate system
# perims_sub_21 = st_transform(perims_sub_21, 5070)  
# 
# #union overlapping polygons
# perims_21_union = st_union(perims_sub_21, by_feature = TRUE)
# 
# perims_21_union = st_cast(st_union(perims_sub_21, by_feature = TRUE), "POLYGON")
