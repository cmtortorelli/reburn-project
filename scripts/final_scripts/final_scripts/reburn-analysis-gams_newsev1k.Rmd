...
count_unique_reburns = data_thin %>%
  ...
  group_by(ecoregion) %>%
  summarise(n = n())
sum(count_unique_reburns$n) #881
...
#count high sev - 2.24
highsev = data_thin %>% group_by(ecoregion) %>%
  summarise(highsev = sum(cbi_reburn > 2.24),
            prophighsev = highsev/n()*100)

data_thin %>% group_by(ecoregion) %>% summarise(med_t_between = median(time_betwe) )
...
data_thin_scale = data_thin %>% scale_df() %>%
  mutate(CT_lumped_codes4 = factor(CT_lumped_codes4))
...
fit_ca = bam(cbi_reburn ~ count_burn + 
...
fit_nm = bam(cbi_reburn ~ count_burn + 
                  s(PRISM_ppt_, k = 5, bs = "cs") +
                  s(PRISM_tmea, k = 5, bs = "cs") +
                  s(cbi_initial, k = 5, bs = "cs") +
                  s(time_betwe, k = 5, bs = "cs") +
...
fit_sw = bam(cbi_reburn ~ count_burn + 
                  s(PRISM_ppt_, k = 5, bs = "cs") +
                  s(PRISM_tmea, k = 5, bs = "cs") +
                  s(cbi_initial, k = 5, bs = "cs") +
                  s(time_betwe, k = 5, bs = "cs") +
...
fit_wm = bam(cbi_reburn ~ count_burn + 
                  s(PRISM_ppt_, k = 5, bs = "cs") +
                  s(PRISM_tmea, k = 5, bs = "cs") +
                  s(cbi_initial, k = 5, bs = "cs") +
                  s(time_betwe, k = 5, bs = "cs") +
...
vca = plot(variogram(resid_ca~1, data=ca_proj, cutoff = 5), main = "California Coast")
vnm = plot(variogram(resid_nm~1, data=nm_proj, cutoff = 5), main = "Northern Mountains")
vsw = plot(variogram(resid_sw~1, data=sw_proj, cutoff = 5), main = "Southwest")
vwm = plot(variogram(resid_wm~1, data=wm_proj, cutoff = 5), main = "Western Mountains")
...
# wordreg(list(fit_ca,fit_nm,fit_sw, fit_wm),
#        caption="GAM output by ecoregion",
#        dcolumn=FALSE,
#        model.names=c("California Coast","Northern Mountains","Southwest", "Western Mountains"),
#        file = "GAM_table.doc")
...
```{r}
saveRDS(fit_ca, datadir("final-models/fit_ca_newsev1k.rds"))
saveRDS(fit_sw, datadir("final-models/fit_sw_newsev1k.rds"))
saveRDS(fit_nm, datadir("final-models/fit_nm_newsev1k.rds"))
saveRDS(fit_wm, datadir("final-models/fit_wm_newsev1k.rds"))
