...
activities = st_intersects(points %>% st_transform(st_crs(facts_sub)),facts_sub)

```

### Get only FACTS management that occured after the intial fire and before the reburn
```{r post-fire management}
# Initiate new columns to store FACTS management history data
points$facts_managed = NA
points$facts_year = NA
points$facts_year_mostrecent = NA
points$facts_activity = NA
...
for(i in 1:nrow(points)) {
  ...
  facts_overlap = facts_sub[facts_indices,]
  
  #keep only years that fall between initial and reburn fire years
  facts_keep = facts_overlap %>% filter(year > plot$secondrece & year < plot$mostrecent)
  
  # get the most recent year of management out of all the facts polygons that occured between the two fire years
  facts_year_mostrecent = max(facts_keep$year)
  ...
  points[i,which(names(points) == "facts_managed")] = TRUE
  points[i,which(names(points) == "facts_year")] = paste(facts_keep$year,collapse=", ")
  points[i,which(names(points) == "facts_year_mostrecent")] = facts_year_mostrecent
  points[i,which(names(points) == "facts_activity")] = paste(facts_keep$NEW_ACTIVITY,collapse=", ")
}
