...
data_lowmod_ca = data_thin %>% filter(ecoregion == "California Coast") %>%
  scale_df()

data_lowmod_nm = data_thin %>% filter(ecoregion == "Northern Mountains") %>%
  scale_df()

data_lowmod_sw = data_thin %>% filter(ecoregion == "Southwest") %>%
  scale_df()

data_lowmod_wm = data_thin %>% filter(ecoregion == "Western Mountains") %>%
  scale_df()
...
gm = function(df, k){
  ...
  gamfit = gam(cbi_reburn ~ count_burn + #not logit transformed response
                  ...
                  ti(time_betwe, 
                     by = PRISM_ppt_, k = k, bs = "cs") +
                  ti(time_betwe, 
                     by = PRISM_tmea, k = k, bs = "cs") +
                  ti(time_betwe, 
                     by = vpd_perc, k = k, bs = "cs") +
                  ti(time_betwe, 
                     by = vs_perc, k = k, bs = "cs") +
                  ti(time_betwe, 
                     by = erc_perc, k = k, bs = "cs") +
                  ti(time_betwe, 
                     by = cbi_initial, k = k, bs = "cs"),
...
fit3_ca = gm(data_lowmod_ca, k = 5)
fit3_nm = gm(data_lowmod_nm, k = 5)
fit3_sw = gm(data_lowmod_sw, k = 5)
fit3_wm = gm(data_lowmod_wm, k = 5)
...
anova(fit3_ca, fit4_ca, test = "Chisq") #sig, increases % dev explained by 1, from 37.5 to 38.5
anova(fit3_nm, fit4_nm, test = "Chisq") #sig, increases % dev explained by 4, from 32.8 to 33.2
anova(fit3_sw, fit4_sw, test = "Chisq") #sig, increases % dev explained by 4, from 25.1 to 28.9
anova(fit3_wm, fit4_wm, test = "Chisq") #sig, increases % dev explained by ~, from 22.7 to 24.3

#compare models with and without interactions
anova(fit3_ca, fit5_ca, test = "Chisq") #sig, decreases to 36.5 (1)
anova(fit3_nm, fit5_nm, test = "Chisq") #sig, decreases to 31.2 (1.6)
anova(fit3_sw, fit5_sw, test = "Chisq") #sig, decreases to 23.2 (1.9)
anova(fit3_wm, fit5_wm, test = "Chisq") #sig, decreases to 18.6 (4.1)
...
ca_preds = erc_preds_func(fit3_ca, data_lowmod_ca)
sw_preds = erc_preds_func(fit3_sw, data_lowmod_sw)
nm_preds = erc_preds_func(fit3_nm, data_lowmod_nm)
wm_preds = erc_preds_func(fit3_wm, data_lowmod_wm)
...
preds_dfw_func = function(df, fit){
  df_w97 = data.frame(count_burn = 2, 
                       # AET = 0, 
                      ...
                      # afg_rebrn = 0, pfg_rebrn = 0,
                       ...
                       lat = mean(df$lat),
                       long = mean(df$long))
  df_w90 = data.frame(count_burn = 2, 
                       # AET = 0, 
                       PRISM_ppt_ = 0, PRISM_tmea = 0,
                       cbi_initial = 0, heat_load = 0, TPI_1024 = 0,
                      # afg_rebrn = 0, pfg_rebrn = 0,
                       #set weather variables to mean 
                       vpd_perc = .5, erc_perc = .5, 
                       vs_perc = 0,
                       #set time between fires to full range 1-25 years
                      time_betwe = seq(min(df$time_betwe), max(df$time_betwe), by = .1),
                      weather = "90perc-.5sd",
                       ...
                       lat = mean(df$lat),
                       long = mean(df$long))
  df_w80 = data.frame(count_burn = 2, 
                       # AET = 0, 
                       PRISM_ppt_ = 0, PRISM_tmea = 0,
                       cbi_initial = 0, heat_load = 0, TPI_1024 = 0,
                      # afg_rebrn = 0, pfg_rebrn = 0,
...
Unscale predictions for data vis
```{r}
predictor_plot_func = function(df_preds){  
  df_unscale = df_preds %>%
    mutate(time_betwe_unscale = (time_betwe * sd(time_betwe)) + mean(time_betwe),
           cbi_initial_unscale = (cbi_initial * sd(cbi_initial)) + mean(cbi_initial),
           erc_perc_unscale = (erc_perc * sd(erc_perc)) + mean(erc_perc),
           vpd_perc_unscale = (vpd_perc * sd(vpd_perc)) + mean(vpd_perc))
...
predictor_plot_func = function(df_preds, df_raw, group_var){
  p = ggplot(data = df_preds, aes(x = time_betwe, y = cbi_reburn, color = group_var)) +
         geom_line(size = 1.05)+
  ...
  p2 = p + geom_rug(data = df_raw, aes(x = time_betwe, y = cbi_reburn), 
                    color = "grey", alpha = .3, position = "jitter", length = unit(0.05, "npc"))
  return(p2)

}

pca = predictor_plot_func(preds_w_ca, data_lowmod_ca, preds_w_ca$weather) + 
  ggtitle("California Coast") + xlab("") +
  theme(legend.position = "none")    

psw = predictor_plot_func(preds_w_sw, data_lowmod_sw, preds_w_sw$weather) + 
  ggtitle("Southwest") + xlab("") + ylab("")

pnm = predictor_plot_func(preds_w_nm, data_lowmod_nm, preds_w_nm$weather) + 
  ggtitle("Northern Mountains") +
  theme(legend.position = "none")    

pwm = predictor_plot_func(preds_w_wm, data_lowmod_wm, preds_w_wm$weather) + 
