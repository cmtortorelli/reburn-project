...
data_thin = data_thin %>% filter(cbi_initial <= 2.25)
```
...
library(ggridges)
#climate space
slice(data_thin, sample(1:n())) %>% #shuffle rows for better visualization
...
library(formattable)
veg = data_thin %>% group_by(ecoregion, CT_lumped_codes3) %>%
  summarise(n = n()) %>% 
...
preds_dfw_func = function(df, fit, vpd_mean, vpd_sd, erc_mean, erc_sd){
  df_w97 = data.frame(count_burn = 2, #unscaled
                      ...
                      time_betwe = seq(min(df$time_betwe), max(df$time_betwe), by = .05),
                      weather = "extreme")
...
preds_dfcbi_func = function(df, fit, cbi_mean, cbi_sd){
  df_lowcbi = data.frame(count_burn = 2, #unscaled
                      PRISM_ppt_ = 0, 
                      PRISM_tmea = 0,
                      cbi_initial = as.numeric((0.51-cbi_mean)/cbi_sd), 
                      heat_load = 0, 
                      TPI_1024 = 0,
                      # AET = 0, 
                      # afg_rebrn = 0, pfg_rebrn = 0,
  ...
  df_lowmodcbi = data.frame(count_burn = 2, #unscaled
                      PRISM_ppt_ = 0, 
                      PRISM_tmea = 0,
                      cbi_initial = as.numeric((1.5-cbi_mean)/cbi_sd), 
                      heat_load = 0, 
                      TPI_1024 = 0,
                      vpd_perc = 0,
                      erc_perc = 0,
  ...
  data_w = rbind(df_lowcbi, df_lowmodcbi, df_modcbi)
...
bins = data_thin %>%
  mutate(cbi_bins = case_when(cbi_initial <= 1.25 ~ "low",
                             cbi_initial > 1.25 & cbi_initial <= 2 ~ "lowmod",
                             cbi_initial > 2 ~ "mod"), 
...
preds_cbi_ca_trunc = preds_cbi_ca %>% 
  mutate(time_betwe_round = round(time_betwe_unscale, 1)) %>%
...
preds_cbi_nm_trunc = preds_cbi_nm %>% 
  mutate(time_betwe_round = round(time_betwe_unscale, 1)) %>%
  pivot_wider(names_from = cbii_cat, values_from = time_betwe_round) %>%
  filter(low >= .1 & low <= 3.4 |
           lowmod >= .1 & lowmod <= 3.6 |
           mod >= .1 & mod <= 3.6) %>%
  pivot_longer(low:mod, names_to = "cbii_cat", values_to = "time_betwe_round") %>%
  na.omit()

preds_cbi_sw_trunc = preds_cbi_sw %>% 
  mutate(time_betwe_round = round(time_betwe_unscale, 1)) %>%
  pivot_wider(names_from = cbii_cat, values_from = time_betwe_round) %>%
  filter(low >= .1 & low <= 2.7 |
           lowmod >= .1 & lowmod <= 3.3 |
           mod >= .1 & mod <= 2.9) %>%
  pivot_longer(low:mod, names_to = "cbii_cat", values_to = "time_betwe_round") %>%
  na.omit()

preds_cbi_wm_trunc = preds_cbi_wm %>% 
  mutate(time_betwe_round = round(time_betwe_unscale, 1)) %>%
  pivot_wider(names_from = cbii_cat, values_from = time_betwe_round) %>%
  filter(low >= .2 & low <= 3.6 |
           lowmod >= .1 & lowmod <= 3.6 |
           mod >= .1 & mod <= 3.6) %>%
  pivot_longer(low:mod, names_to = "cbii_cat", values_to = "time_betwe_round") %>%
  na.omit()
...
predictor_plot_func2 = function(df_preds, df_raw, group_var){
    group_var = recode_factor(group_var, low = "low severity (CBI = 0.51)",
                                 lowmod = "low/moderate severity (CBI = 1.5)",
                                 mod = "moderate severity (CBI = 2.25)") 
...
predictor_plot_func3 = function(df_preds, df_raw, group_var){
    group_var = recode_factor(group_var, coldwet = "cool wet",
                              hotwet = "hot wet",
                              hotdry= "hot dry") 
