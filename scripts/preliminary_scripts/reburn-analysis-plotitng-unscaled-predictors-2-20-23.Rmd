...
preds_dfcbi_func = function(df, fit, cbi_mean, cbi_sd){
  ...
  df_modcbi = data.frame(count_burn = 2, #unscaled
                      PRISM_ppt_ = 0, 
                      PRISM_tmea = 0,
                      cbi_initial = as.numeric((2.5-cbi_mean)/cbi_sd), 
                      heat_load = 0, 
                      TPI_1024 = 0,
                      vpd_perc = 0,
                      erc_perc = 0,
...
bins = data_thin %>%
  mutate(cbi_bins = case_when(cbi_initial <= 1 ~ "low",
                             cbi_initial > 1 & cbi_initial <= 2 ~ "lowmod",
                             cbi_initial > 2 ~ "mod"), 
...
predictor_plot_func2 = function(df_preds, df_raw, group_var){
    group_var = recode_factor(group_var, low = "low severity (CBI = 0.5)",
                                 lowmod = "low/moderate severity (CBI = 1.5)",
                                 mod = "moderate severity (CBI = 2.5)") 
